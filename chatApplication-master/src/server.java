...
public class server {
    ...
    public static final int BUFFSZ = 400;
    public static final int BUFFSHARED = 200;
    ...
    private int port;
      public volatile static  ArrayList<String> messageList = new ArrayList<String>();
public static volatile int arraySize = 0;
    ...
    /******************** The server thread class **************************/
     class ServiceThread extends Thread {
        ...
        public volatile  int messageAmount = 0;
        ...
        public synchronized int getMessageAmount(){return messageAmount;}
        public synchronized void getIncrementMessageAmount(){messageAmount ++;}
        ...
        public void run() {
            ...
            Thread t1 = new Thread(new Runnable() {
                public void run() {
                            ...
                            try {
                                sendMsg(output, messageList.get(messageAmount).toString());
                                getIncrementMessageAmount();
           ...
           t1.start();
            ...
            try {
                ...
                while(running) { //Service thread duty cycle
                    ...
                    do {
                        System.out.println("get fucked2");
                       
                       
                           
                   
                   
                        System.out.println(messageAmount + "message amount");
                        System.out.println(messageList.size() + "size of array");
                        len = input.read(buff);
                        System.out.printf("%d bytes received\n", len);
                        displayBuffer(buff, len);
                        recStg += new String(buff, 0, len);
                        System.out.println(getMessageAmount());
System.out.println(messageList.size());
                    } while (!recStg.contains(endMkr));
                    ...
                    else if(recData[0].equals("MSG"))
                    {
                        String toSend = getUser() + ": " + recData[1];
                        assignToShared(toSend);
 
                    }
                   
                    else if(recData[0].equals("SIGN"))
                    {
            ...
            catch(EOFException ex)
{
            ...
            try {
                input.close();
                output.close(); conn.close(); pw.close(); pw1.close();
    ...
    public synchronized void assignToShared(String shared)
    {
        messageList.add(shared);
        setSize();
